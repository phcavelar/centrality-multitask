...
def model_builder(
  ...
  def _vote_while_cond(i, n_acc, array_accuracies, array_costs):
    return tf.less( i, p )
  #end _vote_while_cond

  def _vote_while_body(i, n_acc, array_accuracies, array_costs):
    ...
    return tf.add( i, tf.constant( 1 ) ), tf.add( n_acc, nodes_n[i] ), array_accuracies, array_costs
  ...
  _, _, array_accuracies, array_costs = tf.while_loop(
    _vote_while_cond,
    _vote_while_body,
    [ tf.constant( 0, dtype = tf.int32 ), tf.constant( 0, dtype = tf.int32 ), array_accuracies, array_costs ]
  )
