...
class InstanceLoader(object):
  ...
  def get_instances(self, n_instances):
    for i in range(n_instances):
      ...
      degree_compare = build_node_comparison_M_from_graph( G, "d_p" )
      betweenness = [ G.nodes[node]["b_p"] for node in G.nodes ]
      betweenness_compare = build_node_comparison_M_from_graph( G, "b_p" )
      closeness = [ G.nodes[node]["c_p"] for node in G.nodes ]
      closeness_compare = build_node_comparison_M_from_graph( G, "c_p" )
      eigenvector = [ G.nodes[node]["e_n"] for node in G.nodes ]
      eigenvector_compare = build_node_comparison_M_from_graph( G, "e_n" )
      yield M,degree,degree_compare,betweenness,betweenness_compare,closeness,closeness_compare,eigenvector,eigenvector_compare, fname
      self.index += 1
    #end
  #end
...
def create_batch(problems):
  ...
  for p in problems:
    if p is None:
      continue
    #end if
    M,degree,degree_compare,betweenness,betweenness_compare,closeness,closeness_compare,eigenvector,eigenvector_compare, fname = p
    fnames.append( fname )
