...
def normalisation( centrality_tensor, centrality, n ):
  if centrality == "eigenvector" or centrality == "closeness":
    return centrality_tensor
  elif centrality == "betweenness":
    return centrality_tensor * 2.0 / ((n-1)*(n-2))
  else:
    return centrality_tensor / n
  #end if-elses
#end normalisation
...
def model_builder(
  ...
  def _vote_while_cond(i, n_acc, array_accuracies, array_costs, array_predictions_normalised):
    return tf.less( i, p )
  #end _vote_while_cond

  def _vote_while_body(i, n_acc, array_accuracies, array_costs, array_predictions_normalised):
    ...
    problem_accuracies = [
      tf.metrics.mean_relative_error(
        ...
        predictions = p_Cs_norm[ centrality_dict[ centrality ] ],
        normalizer = problem_labels[ centrality_dict[ centrality ] ]
      )
      for centrality in centralities
    ]
    ...
    array_predictions_normalised = [
      array_predictions_normalised[ centrality_dict[ centrality ] ].write( i, p_Cs_norm[ centrality_dict[ centrality ] ] )
      for centrality in centralities
    ]
    
    return tf.add( i, tf.constant( 1 ) ), tf.add( n_acc, nodes_n[i] ), array_accuracies, array_costs, array_predictions_normalised
  ...
  _, _, array_accuracies, array_costs, array_predictions_normalised = tf.while_loop(
    _vote_while_cond,
    _vote_while_body,
    [ tf.constant( 0, dtype = tf.int32 ), tf.constant( 0, dtype = tf.int32 ), array_accuracies, array_costs, array_predictions_normalised ]
  )
  ...
  for centrality in centralities:
    ...
    model_dict["{}_predictions_".format(centrality)]  = Cs[ centrality_dict[centrality] ]
    model_dict["{}_predictions".format(centrality)]   = array_predictions_normalised[ centrality_dict[centrality] ].concat()
  #end for
...
def gather_values( values, problem_n ):
  logits_list = list()
  i_acc = 0
  for n in problem_n:
    logits = np.zeros( [n] )
    for i in range(n):
      logits[i] = flattened_values[ i_acc + i ]
    #end for
...
def separate_batch( batch_predictions, problem_n ):
  problem_list = list()
  n_acc = 0
  for n in problem_n:
    problem = batch_predictions[ n_acc : n_acc + n ].copy()
