...
#    intra_op_parallelism_threads = 1
  ...
  with tf.Session(config=config) as sess:
    ...
    # Init error vector
    errors = np.zeros(len(problem_sizes))
    eigerr = np.zeros(len(problem_sizes))
    ...
    for i,n in enumerate(problem_sizes):
      ...
      errors[i] = np.mean( [ total_test_metrics_dict["{c}_REL".format(c=c)] for c in centralities if c != "eigenvector" ] )
      if "eigenvector" in centralities:
        eigerr[i] = total_test_metrics_dict["eigenvector_REL"]
      #
    #end for
    # Get the ylim
    ...
    ax.plot(problem_sizes, 100*errors, linestyle='-', marker='o', color = "#8A4F7D")
    ...
    ax.plot(problem_sizes, 100*errors, linestyle='-', marker='o', color = "#8A4F7D")
    plt.savefig('test-varying-sizes.eps', format='eps', dpi=200)
    ...
    if "eigenvector" in centralities:
      ...
      ax.plot(problem_sizes, eigerr, linestyle='-', marker='o', color = "#8A4F7D")
      ymin, ymax = ax.get_ylim()
      plt.close()
      ...
      ax.plot(problem_sizes, eigerr, linestyle='-', marker='o', color = "#8A4F7D")
      plt.savefig('test-varying-sizes-eig.eps', format='eps', dpi=200)
  #end
#end
