...
class InstanceLoader(object):
  ...
  def get_instances(self, n_instances):
    for i in range(n_instances):
      ...
      degree = [ G.nodes[node]["d_p"] for node in G.nodes ]
      ...
      yield M,degree,fname#,degree_compare,betweenness,betweenness_compare,closeness,closeness_compare,eigenvector,eigenvector_compare, fname
      self.index += 1
    #end
  #end
...
def create_batch(problems):
  ...
  for p in problems:
    if p is None:
      continue
    #end if
    M,degree,fname=p#degree_compare,betweenness,betweenness_compare,closeness,closeness_compare,eigenvector,eigenvector_compare, fname = p
    fnames.append( fname )
...
if __name__ == '__main__':
  ...
  print( "fname", "n", "m", "min", "max", "avg1", "avg2", sep="\t" )
  for b, batch in enumerate( instance_loader.get_batches(1) ):
    degrees = batch["degree"][0]
    print( batch["fnames"][0], batch["problem_n"][0], batch["problem_m"][0], int( min(degrees )), int( max(degrees) ), sum(degrees)/len(degrees), sum(degrees)/batch["problem_n"][0], sep="\t" )
  #end
#end
