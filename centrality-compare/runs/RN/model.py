...
def model_builder(
  ...
  def _vote_while_cond(i, n_acc, array_accuracies, array_costs, logits_matrices, probabilities_matrices):
    return tf.less( i, p )
  #end _vote_while_cond

  def _vote_while_body(i, n_acc, array_accuracies, array_costs, logits_matrices, probabilities_matrices):
    ...
    problem_accuracies = [
      tf.reduce_mean(
        tf.cast(
          tf.equal(
            tf.round(pred_matrices_prob[ centrality_dict[ centrality ] ]), problem_labels[ centrality_dict[ centrality ] ]
          ),
          tf.float32
        )
      )
    ...
    problem_costs = [
      tf.losses.sigmoid_cross_entropy(
        multi_class_labels = problem_labels[ centrality_dict[ centrality ] ],
        logits = pred_matrices[ centrality_dict[ centrality ] ]
      )
      for centrality in centralities
    ]
    ...
    logits_matrices = [
        logits_matrices[centrality_dict[centrality]].write(
            i,
    ...
    probabilities_matrices = [
        probabilities_matrices[centrality_dict[centrality]].write(
            i,
            tf.reshape( pred_matrices_prob[centrality_dict[centrality]], [-1] )
        )
        for centrality in centralities
    ]
    
    return tf.add( i, tf.constant( 1 ) ), tf.add( n_acc, nodes_n[i] ), array_accuracies, array_costs, logits_matrices, probabilities_matrices
  ...
  _, _, array_accuracies, array_costs, logits_matrices, probabilities_matrices = tf.while_loop(
    _vote_while_cond,
    _vote_while_body,
    [ tf.constant( 0, dtype = tf.int32 ), tf.constant( 0, dtype = tf.int32 ), array_accuracies, array_costs, logits_matrices, probabilities_matrices ]
  )
  ...
  for centrality in centralities:
    ...
    model_dict["{}_logits".format(centrality)]        = logits_matrices[ centrality_dict[centrality] ].concat()
    model_dict["{}_probabilities".format(centrality)] = probabilities_matrices[ centrality_dict[centrality] ].concat()
  #end for
