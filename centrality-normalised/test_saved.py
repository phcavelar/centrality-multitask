...
import pprint, sys, model, random, util, time, metrics
...
if __name__ == "__main__":
  ...
  with tf.Session(config=config) as sess:
    ...
    batch = training_instance_loader.get_batch( 32 )
    ...
    for probabilities, tfacc, c in zip( centrality_probabilities, tfaccs, centralities ):
      ...
      for prd, lbl in zip(
        ...
        bACC += ACC
        bnpeqacc += np.mean( np.equal( prd.astype( np.bool ), lbl.astype( np.bool) ).astype( np.float ) )
      #end for
      bACC /= len( batch["problem_n"] )
      bnpeqacc /= len( batch["problem_n"] )
      
      pprint.pprint(
        list(
          zip(
            ["c", "tfacc", "bACC", "ACC", "bnpeqacc"],
            [ c ,  tfacc ,  bACC ,  ACC ,  bnpeqacc ]
          )
        )
      )
