...
def run_batch(
  ...
  feed_dict = {
    ...
    **{
      model_dict["{c}_cmp_labels".format(c=centrality)]: cmp_labels[centrality]
      for centrality in centralities
    }
  }
  ...
  returned_comparisons = session.run(
    [
      model_dict["{c}_cmp".format( c = centrality ) ]
      for centrality in centralities
    ],
    feed_dict = feed_dict
  )
  ...
  for centrality, predictions, cost in zip( centralities, returned_comparisons, returned_losses[1:] ):
